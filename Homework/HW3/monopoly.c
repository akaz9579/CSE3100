...
int one_round(int m, int n, TPlayer p[], TProperty prop[]){
	for(int i = 0; i < m; i++)
	{
		...
		int currStep = p[1].loc + steps;
		if (currStep > n){
			currStep -=n;
			p[i].balance += (double)n;
		}
		if(prop[currStep].owner_id == -1){
			prop[currStep].owner_id =p[i].id;
		}else if(prop[currStep].owner_id != -1 && prop[currStep].owner_id !=p[i].id){
			int suc = transaction(&p[i], &p[prop[currStep].owner_id],prop[currStep].rent);
			if(suc==0){
				return 0;
			}
		}
