...
{   
	...
	while(!done)
	{
		...
		if(p->head == NULL){
			...
			node * removed = remove_first(&p->head, &p->tail);
			prepare(removed->v);
			add_to_buffer(removed->v, removed->v, q);
			my_data->total = my_data->total + 1;
			pthread_mutex_unlock(&p->mutex);
			free(removed);
		}
	}
