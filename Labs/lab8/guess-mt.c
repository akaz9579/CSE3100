...
void * thread_c(void * arg_in)
{
    ...
    check_pthread_return(pthread_mutex_lock(&arg->mutex), "Failed to lock mutex C");
    arg->max = gmn_get_max();
    arg->status = S_MAX;
    check_pthread_return(pthread_cond_signal(&arg->cond_result), "Failed to signal cond_result C");
    while(1){
        while (arg->status != S_GUESS){
            check_pthread_return(pthread_cond_wait(&arg->cond_guess, &arg->mutex), "Failed to wait on cond_guess C");
        }
        ...
        arg->status = S_RESULT;
        check_pthread_return(pthread_cond_signal(&arg->cond_result), "failed to signal cond_result C");
         check_pthread_return(pthread_mutex_unlock(&arg->mutex), "Failed to unlock mutex C");
...
void * thread_p(void *arg_in)
{
    ...
    check_pthread_return(pthread_mutex_lock(&arg->mutex), "failed to lock mutex P"); 
    while(arg->status != S_MAX){
        check_pthread_return(pthread_cond_wait(&arg->cond_result, &arg->mutex), "Failed to wait on con");
    }
    max = arg->max;
    ...
    do { 
        ...
        //     wait for the result from thread_c (copy arg->result to result!)
             arg->guess = guess;
        arg->status = S_GUESS;
        check_pthread_return(pthread_cond_signal(&arg->cond_guess), "Failed to signal cond_guess P");

        while (arg->status != S_RESULT){
            check_pthread_return(pthread_cond_wait(&arg->cond_result, &arg->mutex), "Failed");
        }
        result = arg->result; 
...
int main(int argc, char *argv[])
{
    ...
    if (pthread_cond_init(&arg.cond_guess, NULL) != 0)
        die("cond_guess init failed");
    if (pthread_cond_init(&arg.cond_result, NULL) != 0)
        die("cond_result init failed");
